def particionada_reubicable(procesos, memoria):
    ...
    asignados = [[] for _ in range(len(memoria))]  # Lista de listas para almacenar los procesos asignados a cada bloque
    procesos_asignados = 0  # Contador de procesos asignados
    trabajos_postergados = 0  # Contador de trabajos no asignados
    
    # Iterar sobre los procesos para asignarlos
    for i, proceso in enumerate(procesos):
        asignado = False
        print(f"\nIntentando asignar el proceso {proceso}MB")
        
        # Primero intentar asignar en los bloques de memoria disponibles
        for j, bloque in enumerate(espacio_restante):
            if espacio_restante[j] >= proceso:  # Si hay suficiente espacio
                print(f"Proceso {proceso}MB entra en el bloque {j+1} con {espacio_restante[j]}MB de espacio restante.")
                espacio_restante[j] -= proceso  # Reducir el espacio en el bloque
                asignados[j].append(proceso)  # Agregar el proceso al bloque correspondiente
                asignado = True
                procesos_asignados += 1
                break
        
        # Si el proceso no pudo ser asignado a ningún bloque inicialmente
        if not asignado:
            print(f"Proceso {proceso}MB no pudo ser asignado a ningún bloque.")
            trabajos_postergados += 1
            for j in range(len(espacio_restante)):
                if espacio_restante[j] > 0:
                    # Reubicar procesos para hacer espacio
                    for k in range(len(procesos)):
                        if procesos[k] <= espacio_restante[j] and procesos[k] != 0:
                            print(f"Reubicando el proceso {procesos[k]}MB para hacer espacio en el bloque {j+1}.")
                            espacio_restante[j] -= procesos[k]
                            procesos[k] = 0  # El proceso ha sido reubicado
                            break
                    if espacio_restante[j] >= proceso:
                        print(f"Proceso {proceso}MB ahora entra en el bloque {j+1}.")
    ...
    print("\n======================= Resultados =======================")
    for i, bloque in enumerate(asignados):
        # Imprimir los procesos asignados a cada bloque
        print(f"Bloque {i+1}: {', '.join(map(str, bloque)) if bloque else 'No asignado'}")
    
    # Calcular y mostrar la memoria desperdiciada
    memoria_desperdiciada = sum(espacio_restante)
    
    # Calcular las métricas
    memoria_utilizada = sum(memoria) - memoria_desperdiciada
    fragmentacion_externa = memoria_desperdiciada
    porcentaje_uso_memoria = (memoria_utilizada / sum(memoria)) * 100
    nivel_multiprogramacion = procesos_asignados

    # Mostrar las métricas
    print(f"\nFragmentación externa: {fragmentacion_externa}MB")
    ...
    # Mostrar memoria desperdiciada
    print(f"Memoria desperdiciada: {memoria_desperdiciada}MB")
...
# Llamar a la función con las listas definidas
particionada_reubicable(procesos, memoria)
